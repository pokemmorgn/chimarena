...
/**
 * 🎮 CONFIGURATION COLYSEUS COMPLÈTE CORRIGÉE
 */
async function setupColyseus(app: express.Application, config: any): Promise<void> {
  try {
    ...
    // ✅ CONFIGURATION COLYSEUS MODERNE (sans warnings)
    const { WebSocketTransport } = require("@colyseus/ws-transport");
    
    // Créer le serveur Colyseus avec la nouvelle configuration
    ...
    // ✅ DÉMARRER LE SERVEUR COLYSEUS (correction)
    console.log(`🎮 Démarrage du serveur Colyseus sur le port ${config.colyseus.port}...`);
    
    // Démarrer le serveur HTTP avec Colyseus
    await new Promise<void>((resolve, reject) => {
      httpServer.listen(config.colyseus.port, config.host, (error?: Error) => {
        if (error) {
          reject(error);
        } else {
