...
private async handleSearchBattle(client: Client, player: WorldPlayer) {
  ...
  try {
    ...
    const user = await UserModel.findById(player.userId).select('deck currentArenaId cards');
    if (!user) {
...
private async handleMatchFound(match: MatchResult): Promise<void> {
  ...
  try {
    ...
    if (client1) {
      ...
      // Récupérer les vraies données utilisateur validées
      const user1 = await UserModel.findById(match.player1.userId).select('username playerStats deck');
      ...
      client1.send("match_found", {
        ...
        playerData: {
          ...
          username: user1?.username || match.player1.username,
          level: user1?.playerStats?.level || match.player1.level,
          trophies: user1?.playerStats?.trophies || match.player1.trophies,
          deck: user1?.deck || match.player1.deck
        },
        arena: match.arena,
        matchQuality: match.matchQuality
      });
    ...
    if (client2) {
      ...
      // Récupérer les vraies données utilisateur validées
      const user2 = await UserModel.findById(match.player2.userId).select('username playerStats deck');
      ...
      client2.send("match_found", {
        ...
        playerData: {
          ...
          username: user2?.username || match.player2.username,
          level: user2?.playerStats?.level || match.player2.level,
          trophies: user2?.playerStats?.trophies || match.player2.trophies,
          deck: user2?.deck || match.player2.deck
        },
        arena: match.arena,
        matchQuality: match.matchQuality
      });
...
private async handleGetDeckInfo(client: Client, player: WorldPlayer) {
  try {
    ...
    const user = await UserModel.findById(player.userId).select('deck cards currentArenaId');
    if (!user) {
...
private async handleValidateDeck(client: Client, player: WorldPlayer, message: any) {
  try {
    ...
    const user = await UserModel.findById(player.userId).select('currentArenaId');
